# Copyright (C) 2020-2026 CZ.NIC z.s.p.o. (https://www.nic.cz/)
#
# This is free software, licensed under the GNU General Public License v3.
# See /LICENSE for more information.

PROJECT="reForis Matter" 
...
install:
	...
	ln -sf /tmp/reforis-matter/reforis_static/reforis_matter /tmp/reforis/reforis_static/
...
.PHONY: lint-web
lint-web: venv
	$(VENV_BIN)/$(PYTHON) -m ruff check reforis_matter
...
init-langs: create-messages
	for lang in $(LANGS); do \
		...
		-i reforis_matter/translations/messages.pot \
		-d reforis_matter/translations/ -l $$lang \
	; done

.PHONY: create-messages
create-messages: venv
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_matter/translations/messages.pot . --project=$(PROJECT) --version=$(VERSION) --copyright-holder=$(COPYRIGHT_HOLDER) --msgid-bugs-address=$(MSGID_BUGS_ADDRESS)

.PHONY: update-messages
update-messages: venv
	$(VENV_BIN)/pybabel update -i ./reforis_matter/translations/messages.pot -d ./reforis_matter/translations --update-header-comment

.PHONY: compile-messages
compile-messages: venv install-js
	$(VENV_BIN)/pybabel compile -f -d ./reforis_matter/translations
...
clean:
	...
	rm -rf $(JS_DIR)/node_modules/ reforis_static/reforis_matter/js/app.min.js
	$(PYTHON) -m pip uninstall -y reforis_matter
